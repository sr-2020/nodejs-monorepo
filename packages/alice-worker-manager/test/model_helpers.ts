...
export async function getModelAtTimestamp(id: string, timestamp: number, dbAlias: string = 'models') {
  while (true) {
    const model = await getModel(id, dbAlias);
    if (model && 'timestamp' in model && model.timestamp == timestamp) return model;
    await delay(10);
  }
}

export function getModelVariants(id: string, aliases: string[] = ['models', 'workingModels', 'defaultViewModels']) {
  const pending = aliases.map((alias) => getModel(id, alias));
  return Promise.all(pending);
}

export function getModelVariantsAtTimestamp(
  id: string,
  timestamp: number,
