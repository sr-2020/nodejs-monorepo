...
import {
  ...
  SyncRequestsSourceToken,
  ViewModelStorageToken,
  WorkersPoolToken,
  WorkingModelStorageToken,
} from './di_tokens';
...
export function initializeDI(config: Config) {
  ...
  Container.set(WorkingModelStorageToken, new WorkingModelStorage(Container.get(ConfigToken), Container.get(DBConnectorToken)));
  Container.set(ViewModelStorageToken, new ViewModelStorage(Container.get(ConfigToken), Container.get(DBConnectorToken)));
  Container.set(EventStorageToken, eventStorageFactory(Container.get(ConfigToken), Container.get(DBConnectorToken)));
  Container.set(SyncRequestsSourceToken, eventsSourceFactory(Container.get(ConfigToken), Container.get(DBConnectorToken)));
