...
describe('QR Service', () => {
  ...
  it('Decodes encoded code', async () => {
    const encodeResponse = await rp
      .get(address + '/encode?type=10&kind=26&validUntil=1697919090&payload=UUID', {
        json: {},
  ...
  it('Decodes encoded code if type provided by string', async () => {
    const encodeResponse = await rp
      .get(address + '/encode?type=Pill&kind=26&validUntil=1697919090&payload=UUID', {
        json: {},
        auth: { username: 'some_user', password: 'qwerty' },
      })
      .promise();
  ...
  it('Encode fails with credentials for wrong user', async () => {
    const response = await rp
      .get(address + '/encode?type=10&kind=26&validUntil=1697919090&payload=UUID', {
        json: {},
        auth: { username: 'not_user', password: 'qwerty' },
        resolveWithFullResponse: true,
        simple: false,
      })
      .promise();
  ...
  it('Encode fails with credentials with wrong password', async () => {
    const response = await rp
      .get(address + '/encode?type=10&kind=26&validUntil=1697919090&payload=UUID', {
        json: {},
        auth: { username: 'some_user', password: 'asdfg' },
        resolveWithFullResponse: true,
        simple: false,
      })
      .promise();
