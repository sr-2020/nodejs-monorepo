  ...
  describe('Transfer', () => {
    it('Can transfer', async () => {
      ...
      response = await rp
        .post(address + '/economy/transfer', {
          resolveWithFullResponse: true,
          json: { sender: 'second', receiver: 'first', amount: 5, description: 'For dark side' },
    ...
    it('Can process simultaneous transfers', async () => {
      ...
      for (let i = 0; i < 20; ++i) {
        promises.push(
          rp
            .post(address + '/economy/transfer', {
              resolveWithFullResponse: true,
              json: { sender: 'first', receiver: 'second', amount: 10, description: 'For cookies' },
              auth: { username: 'first', password: '1' },
            })
            .promise(),
        );
      ...
      for (const r of responses) expect(r.statusCode).to.eq(200);
