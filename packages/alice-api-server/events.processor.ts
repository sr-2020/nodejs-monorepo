...
export class EventsProcessor {
  ...
  public async process(id: string, request: ProcessRequest): Promise<StatusAndBody> {
    ...
    if (!request.scheduledUpdateTimestamp && request.events.some((event) => event.timestamp <= cutTimestamp))
    ...
    const connection = new Connection(
      this.dbContainer().eventsDb(),
      this.dbContainer().metadataDb(),
      Container.get(ApplicationSettingsToken).viewmodelUpdateTimeout,
    );
    if (request.scheduledUpdateTimestamp) {
      if (this.dbContainer().connections.has(id)) throw new HttpError(429, 'Multiple connections from one client are not allowed');

      this.dbContainer().connections.set(id, connection);
      const s = await this.dbContainer()
        .connections.get(id)
        .processEvents(id, request);
  ...
  public async latestExistingMobileEventTimestamp(id: string): Promise<number> {
    try {
      return (await this.dbContainer()
        .metadataDb()
        .get(id)).scheduledUpdateTimestamp;
    } catch (e) {
      if (e.status == 404) {
        return 0;
      } else {
  ...
  private async processTokenUpdateEvents(id: string, events: CharacterlessEvent[]) {
    ...
    if (tokenUpdatedEvents.length > 0) {
      ...
      const existingCharacterWithThatToken = await this.dbContainer()
        .accountsDb()
        .find({ selector: { pushToken: token } });
      for (const existingCharacter of existingCharacterWithThatToken.docs) {
        await this.dbContainer()
          .accountsDb()
          .upsert(existingCharacter._id, (accountInfo: any) => {
            this.logger.info(
              `Removing token (${token} == ${accountInfo.pushToken}) ` + `from character ${existingCharacter._id} to give it to ${id}`,
            );
  ...
  private _cancelUpdateIfTooFarInFuture(id: string, request: ProcessRequest) {
    ...
    if (
      request.scheduledUpdateTimestamp &&
      tooFarInFutureFilterTime &&
      request.scheduledUpdateTimestamp > currentTimestamp() + tooFarInFutureFilterTime
    ) {
      this.logger.error(`Received request with scheduledUpdateTimestamp too far in future. Somebody messing with time?`, {
        source: 'api',
        characterId: id,
      });
