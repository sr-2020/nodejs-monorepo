...
import { BalancesDocument, DatabasesContainerToken, ProvisionRequest, SetBonusRequest, TransactionRequest } from '../services/db-container';
import { AccessPropagation, canonicalId, checkAccess, currentTimestamp, returnCharacterNotFoundOrRethrow } from '../utils';
...
export class EconomyController {
  ...
  public async transfer(@CurrentUser() user: AliceAccount, @Body() body: TransactionRequest) {
    try {
      body.sender = await canonicalId(body.sender);
      await checkAccess(user, body.sender);
      body.receiver = await canonicalId(body.receiver);
      if (body.amount <= 0) throw new BadRequestError('Величина транзакции должна быть положительной.');
      if (body.sender == body.receiver) throw new BadRequestError('Нельзя переводить деньги самому себе.');

      const db = Container.get(DatabasesContainerToken).economyDb();
      await db.upsert('balances', (doc: any) => {
        if (doc[body.receiver] == undefined) throw new NotFoundError('Получатель не найден');
        if (doc[body.sender] == undefined) throw new NotFoundError('Отправитель не найден');

        if (doc[body.sender] < body.amount) throw new BadRequestError('Недостаточно денег.');
      ...
      await sendGenericPushNotification(
        body.receiver,
        makeVisibleNotificationPayload(
          `Получен платеж: ${body.amount}`,
          `Отправитель платежа: ${body.sender}` +
            (body.description != undefined && body.description.length > 0 ? `, описание: "${body.description}"` : ''),
        ),
      );
      return {};
    } catch (e) {
  ...
  public async setBonus(@CurrentUser() user: AliceAccount, @Body() body: SetBonusRequest) {
    try {
      ...
      await sendGenericPushNotification(body.userId, makeVisibleNotificationPayload(`${action} премия от компании: ${bonuses.join(', ')}`));
      return {};
    } catch (e) {
      returnCharacterNotFoundOrRethrow(e);
    }
  ...
  public async provision(@CurrentUser() user: AliceAccount, @Body() body: ProvisionRequest) {
    try {
      ...
      if (body.initialBalance < 0) throw new BadRequestError('Начальный баланс не может быть отрицательным');

      const target = await Container.get(DatabasesContainerToken)
        .accountsDb()
        .get(userId);

      if (!target) throw new NotFoundError('Не удалось найти пользователя');
  ...
  public async paySalary(@CurrentUser() user: AliceAccount) {
    try {
      ...
      const accounts = await Container.get(DatabasesContainerToken)
        .accountsDb()
  ...
  public async get(@CurrentUser() user: AliceAccount, @Param('id') id: string) {
    ...
    const docs = (await Container.get(DatabasesContainerToken)
      ...
      (await Container.get(DatabasesContainerToken)
        .economyDb()
        .find({
          selector: {
            receiver: id,
