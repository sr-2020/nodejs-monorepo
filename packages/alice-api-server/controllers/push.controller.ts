...
import {
  ...
  sendGenericPushNotificationThrowOnError,
} from '../push-helpers';
...
export class PushController {
  ...
  public async postVisible(@CurrentUser() user: AliceAccount, @Param('id') id: string, @Body() notification: Notification) {
    await checkAccess(user, id, AccessPropagation.AdminOnly);
    return await sendGenericPushNotificationThrowOnError(
      await canonicalId(id),
  ...
  public async postRefresh(@CurrentUser() user: AliceAccount, @Param('id') id: string) {
    await checkAccess(user, id, AccessPropagation.AdminOnly);
    return await sendGenericPushNotificationThrowOnError(await canonicalId(id), makeSilentRefreshNotificationPayload());
  }
