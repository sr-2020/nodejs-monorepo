...
const kAllSpellsList: Spell[] = [
...

  {
    ...
    // 520
    // заклинание на Т1=Мощь*3 минут дает цели защиту от тяжелого оружия. Маг отдает заказчику красную ленту (как маркер защиты от тяжелого оружия из правил по боевке) и привязанный к ней qr-код (как активатор артефакта, и
