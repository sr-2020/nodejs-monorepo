...
const kAllSpellsList: Spell[] = [
...

  {
    ...
    description:
      'Создать временную способность "магический щит" (прозрачный зонтик, защищает от любого легкого оружия), потребуется ее активация перед использованием. Чем больше Мощь, тем дольше сроки готовности и использования',
    // 403
    // у мага на время T1/на одну активацию (что раньше закончится) появляется активируемая способность, при активации которой он в течение времени T2 может защищаться магическим щитом от атак легким оружием - холодным и дистанционным. T1=Мощь*10 минут. Т2=Мощь*1 минут
...

  {
    ...
    description:
      'Мясное/эктоплазменное тело, на которое было активировано заклинание, временно получает способность "какое-то одно оружие в руках считается тяжелым" (потребуется активация эффекта через qr-код и маркировка этого оружия красной ленточкой). Чембольше Мощь, тем дольше сроки готовности и использования',
    // 404
    // заклинание на T1=Мощь*3 минут делает оружие тяжелым. Маг отдает заказчику красную ленту (как маркер тяжелого оружия из правил по боевке) и привязанный к ней qr-код (как активатор артефакта, и он же содержит срок годности T2 = Мощь*20 минут). То есть в течение периода Т2 можно один раз активировать артефакт на какое-то мясо/экто и у него в руках в течение Т1 оружие будет считаться тяжелым (красная лента повязывается на это оружие для синхронности с правилами по боевке)
    // TODO(https://trello.com/c/QrN1vtXh/159-реализовать-заклинания-про-тяжелую-броню-и-тяжелое-оружие-enlarge-your-pencil-и-stone-skin)
    sphere: 'fighting',
    eventType: dummySpell.name,
  },

  {
    ...
    description:
      'Мясное/эктоплазменное тело, на которое было активировано заклинание, временно получает способность "Тяжелая броня" (потребуется активация эффекта через qr-код). Чем больше Мощь, тем дольше сроки готовности и использования',
    // 405
    // заклинание на Т1=Мощь*3 минут дает цели защиту от тяжелого оружия. Маг отдает заказчику красную ленту (как маркер защиты от тяжелого оружия из правил по боевке) и привязанный к ней qr-код (как активатор артефакта, и он же содержит срок годности Т2= Мощь*20 минут). То есть в течение периода Т2 можно один раз активировать артефакт на какое-то мясо/экто и его броня в течение Т1 минут будет защищать от тяжелого оружия (красная лента повязывается на эту броню для синхронности с правилами по боевке)
    // TODO(https://trello.com/c/QrN1vtXh/159-реализовать-заклинания-про-тяжелую-броню-и-тяжелое-оружие-enlarge-your-pencil-и-stone-skin)
    sphere: 'protection',
    eventType: dummySpell.name,
  },

  {
    ...
    // 468
    // разовое уменьшение всем персонажам (мясо/экто/дрон), присутствующим в реале в текущей локации, максимальных хитов на N с задержкой в 30-5*N секунд. N=Мощь/2 (не меньше 1)
...

  {
    ...
    // 469
    // в течение T минут каждые 60 секунд у всех присутствующих в реале  (мясо/экто/дрон) в текущей локации на данный момент максимальные хиты уменьшаются на 1 на срок 30 минут. Если максимальные хиты уменьшились таким образом до нуля, то персонаж оказывается в тяжране. T=Мощь*3
    // TODO(https://trello.com/c/hIHZn9De/154-реализовать-заклинания-бьющие-по-всем-в-текущей-локации)
    sphere: 'fighting',
    eventType: dummyAreaSpell.name,
  },
...

  {
    ...
    // 471
    // после активации заклинания в течение T минут каждые 180 секунд все присутствующие в реале (мясо/экто) в текущей локации на данный момент восстанавливают текущие хиты до максимума. T=Мощь*20
    // TODO(https://trello.com/c/hIHZn9De/154-реализовать-заклинания-бьющие-по-всем-в-текущей-локации)
    sphere: 'healing',
    eventType: dummyAreaSpell.name,
  },

  {
    ...
    // 472
    // после активации заклинания в течение T минут каждые 60 секунд у всех присутствующих в реале (мясо/экто/дрон) в текущей локации текущие хиты уменьшаются на √X. X - число имплантов у каждой конкретной жертвы. T=Мощь*3
    // TODO(https://trello.com/c/hIHZn9De/154-реализовать-заклинания-бьющие-по-всем-в-текущей-локации)
    sphere: 'fighting',
    eventType: dummyAreaSpell.name,
  },

  {
    ...
    // 473
    // после активации заклинания в течение T минут каждые 60 секунд все, присутствующие в реале в этой локации (мясо) И имеющие характеристику Магия>0, получают Откат (дополнительно к имеющемуся), равный их Магии/2 (округленный вверх). Т=Мощь*3
    // TODO(https://trello.com/c/hIHZn9De/154-реализовать-заклинания-бьющие-по-всем-в-текущей-локации)
    sphere: 'fighting',
    eventType: dummyAreaSpell.name,
  },
...

  {
    ...
    // 473
    // После активации заклинания в приложении выводятся текстом данные о заклинаниях, сотворенных в этой локации в последние 60 минут - список (название заклинания,  Мощь, Откат, (20+N)% ауры творца, метарасу творца). N=Мощь*10, но не более 60
    sphere: 'astral',
...

  {
    ...
    // В течение Мощь*8 минут даты активации всех заклинаний с датой "sysdate - 1/24/60/60" каждые 60с сдвигаются в прошлое на 300с минут.
    sphere: 'astral',
...

  {
    ...
    // 539
    // В течение 60*24*2 минут доступно неограниченное число активаций. При активации необходимо сосканировать qr-код мясного тела цели, цель теряет один текущий хит, и если цель подходит под случайным образом выбранный на
...

  {
    ...
    // 495
    // У цели в течение 120 минут маска ауры увеличена на Мощь*2
    sphere: 'aura',
...

  {
    ...
    // 552
    // у цели на время T понижается Харизма на N. T=Мощь*10 минут. N=Мощь-1, но не меньше 1
    sphere: 'stats',
...

  {
    ...
    // 553
    // у цели на время T повышается Харизма на N. T=Мощь*10 минут. N=Мощь-2, но не меньше 1
    sphere: 'stats',
...

  {
    ...
    // 489
    // .  T=Мощь*10 минут. N=Мощь*5, но не меньше 10
...

  {
    ...
    // 490
    // у цели на время T появляется модификатор стоимости всех приобретаемых товаров = [1-(N/100)] (через скоринг?). T=Мощь*10 минут. N=Мощь*5, но не больше 90
    sphere: 'stats',
