...
class ModifierWithAmount extends Modifier {
  amount: number;
}

export function increaseMagic(api: Sr2020CharacterApi, m: ModifierWithAmount) {
  api.model.magic += m.amount;
  api.model.magic = clamp(api.model.magic, -9000, 9000);
}

export function increaseMagicFeedbackReduction(api: Sr2020CharacterApi, m: ModifierWithAmount) {
...
export function increaseMagicRecoverySpeed(api: Sr2020CharacterApi, m: ModifierWithAmount) {
