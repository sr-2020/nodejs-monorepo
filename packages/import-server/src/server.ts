...
if (params.export || params.import || params.id || params.test || params.list || params.refresh || params.econ) {
  ...
  Observable.timer(0, config.importInterval)
    ...
    .subscribe(
      () => winston.info('Finished one import cycle, waiting for next'),
      () => {
        process.exit(1);
      },
      () => {
      ...
      Observable.from([char])
        ...
        .map((result: any) => {
          switch (result.result) {
            ...
            default:
              reject('Unexpected result.result');
          }
          resolve();
        });
    });
...
async function exportCharacterModel(char: JoinCharacterDetail, data: ModelImportData, exportModel = true): Promise<JoinCharacterDetail> {
  ...
  const exporter = new AliceExporter(char, await data.importer.getMetadata(), true, params.ignoreInGame);
  ...
  if (exportResult) {
    ...
    winston.info(`Exported model and account for character ${char._id}, results: ${JSON.stringify(exportResult)}`);
  } else {
    if (char.InGame) {
...
function loadCharactersFromJoin(data: ModelImportData): Observable<JoinCharacterDetail> {
  ...
  return (
    Observable.from(data.charList)
      ...
      .do((c: JoinCharacterDetail) => winston.info(`Imported character: ${c.CharacterId}`))
  ); // Написать в лог
}
...
async function importAndCreate(
  ...
  winston.info(
    `Run import sequence with: id=${id}, import=${importJoin}, export=${exportModel}, ` +
  ...
  if (updatedSince) importData.lastRefreshTime = updatedSince;
  ...
  return new Promise<void>((resolve, reject) => {
    charactersDetails
      ...
      .flatMap((c) => {
        return (async () => {
          await Promise.all(workData.afterConversionProviders.map((provider) => perfromProvide(provider, c, exportModel)));
          return c;
        })();
      })
...
function configureLogger() {
  ...
  winston.add(Elasticsearch, { level: 'debug', indexPrefix: 'importserver-logs', clientOpts: { host: config.log.elasticHost } });
