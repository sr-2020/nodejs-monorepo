...
export async function sendEventToShip() {
  ...
  const response = await rp
    .post(`${address}/location_events/ship_${ship}`, {
      resolveWithFullResponse: true,
      simple: false,
  ...
  if (response.statusCode != 200) {
    ...
    console.info(`Current latency stats: min = ${minLatency}, max = ${maxLatency}, avg = ${totalShipLatency / totalShipRequests}`);
  }
}
