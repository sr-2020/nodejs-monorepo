...
const kAllSpellsList: Spell[] = [
  ...
  // У мага появляется на 3 минуты пассивная абилка с текстом "У всех персонажей, присутствовавших на конец каста заклинания в реале в текущей локации (мясо/экто/дрон - кроме самого мага и тех, кого он вслух укажет) и взаимодействующих с магом (слышащих/видящих/нападающих на него), хиты снижаются на N". N=Мощь/2 (округленное вверх). Пояснение как должно работать - маг сам всем рассказывает, что у них хиты упали, или делает это через мастера. Для подтверждения может показать текст.
  {
    id: 'avalanche',
  ...
  // TODO(https://trello.com/c/hIHZn9De/154-реализовать-заклинания-бьющие-по-всем-в-текущей-локации)
  // На время T минут у мага появляется пассивная абилка без эффекта, но с текстом "Пока действует заклинание каждые 60 секунд у всех присутствующих в реале  (мясо/экто/дрон -  кроме самого мага и тех, кого он вслух укажет) в текущей локации хиты уменьшаются на 1 на срок 30 минут. Если хиты уменьшились таким образом до нуля, то персонаж оказывается в тяжране". T=Мощь*3. <Такое-то время> - момент активации заклинания.
  ...
  // TODO(https://trello.com/c/hIHZn9De/154-реализовать-заклинания-бьющие-по-всем-в-текущей-локации)
  // На время T минут у мага появляется пассивная абилка без эффекта, но с текстом "Пока действует заклинание каждые 60 секунд у всех присутствующих в астрале (кроме самого мага и тех, кого он вслух укажет) в этой локации на данный момент хиты астрального тела (ат) уменьшаются на 1 на срок 30 минут. Если хиты (ат) уменьшились таким образом до нуля, то персонажа с базовым мясным телом выбрасывает из астрала, а у базового астрального случается астральный нокаут (в соответствии с правилами по астральной боевке)". T=Мощь*5. <Такое-то время> - момент активации заклинания.
  // Пояснение как должно работать - маг сам всем рассказывает, что у них хиты падают, или делает это через мастера. Для подтверждения может показать текст.
  {
    id: 'cacophony',
