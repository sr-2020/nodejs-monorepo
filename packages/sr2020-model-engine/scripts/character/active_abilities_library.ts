...
export const kAllActiveAbilitiesList: ActiveAbility[] = [
  ...
  // - время действия 5+N минут, кулдаун 20 минут. Позволяет автоматически подняться из тяжрана через 30с с полным запасом текущих хитов. N=умвл*2 минут
  {
    ...
    description: 'Возможность в течение некоторого периода автоматически один раз подняться из тяжрана. Требуемая эссенция: больше 2',
    target: 'scan',
    targetsSignature: kNoTarget,
    cooldownMinutes: 20,
    prerequisites: ['arch-mage-adeptus'],
  ...
  // - мгновенное, кулдаун 5 минут. Позволяет поднять из тяжрана одного другого персонажа с полным запасом текущих хитов
  {
    ...
    description: 'Поднятие цели из тяжрана. Требуемая эссенция: больше 5',
    target: 'scan',
    targetsSignature: [
      {
        name: 'Персонаж',
  ...
  // Поскольку кулдаун у абилки 0, то маг может продолжать свои попытки ловить духов.
  {
    ...
    description:
      'Попытаться поймать духа, находящегося в этой локации. Потребуется свободное духохранилище. При неудаче будет временный штраф',
    target: 'scan',
  ...
  // На 20 минут выдаётся AstralopithecusRage
  {
    ...
    description: 'Сможешь из реала видеть и изгонять сущности, находящиеся в астрале. Требуемая эссенция: больше 3',
    target: 'scan',
    targetsSignature: kNoTarget,
    cooldownMinutes: 30,
    prerequisites: ['arch-mage-adeptus', 'master-of-the-universe'],
  ...
  // Переход в тело духа совершается на 20 минут, но может быть модифицировано другими абилками
  {
    ...
    description: 'Временно занять тело духа, находящегося в духохранилище',
    target: 'scan',
    targetsSignature: kNoTarget,
    cooldownMinutes: 20,
    prerequisites: ['arch-mage-summoner'],
  ...
  // При активации абилки открывается окно сканирования QR-кода, необходимо сосканировать пустой QR-код. После этого будет создано свободное телохранилище, действующее в течение 600 минут. По окончании этого срока QR-код освободится, и если там находился дух - он окажется на свободе
  {
    ...
    description: 'Создать телохранилище (нужен QR-код пустышка)',
    target: 'scan',
    targetsSignature: kNoTarget,
    cooldownMinutes: 240,
    prerequisites: ['arch-mage-summoner'],
