...
export function increaseMaxMeatHp(api: EffectModelApi<Sr2020Character>, m: ModifierWithAmount) {
  ...
  if (api.model.maxHp == 0 && api.model.healthState != 'clinically_dead' && api.model.healthState != 'biologically_dead') {
...
export function clampAttributes(api: EffectModelApi<Sr2020Character>, m: Modifier) {
  ...
  api.model.hacking.maxTimeAtHost = clamp(api.model.hacking.maxTimeAtHost, 30, 100);
  api.model.hacking.hostEntrySpeed = clamp(api.model.hacking.hostEntrySpeed, 2, 20);
  api.model.hacking.conversionAttack = clamp(api.model.hacking.conversionAttack, 0, 100);
  api.model.hacking.conversionFirewall = clamp(api.model.hacking.conversionFirewall, 0, 100);
  api.model.hacking.conversionSleaze = clamp(api.model.hacking.conversionSleaze, 0, 100);
  api.model.hacking.conversionDataprocessing = clamp(api.model.hacking.conversionDataprocessing, 0, 100);
