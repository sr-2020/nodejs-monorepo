...
class App {
    ...
    constructor(
        ...
        private accountsDb: PouchDB.Database<Account>,
        ...
        const auth = (propagateAccess: boolean) => async (req, res, next) => {
            ...
            if (credentials) {
                try {
                    const thisAccount = await this.findAccountByLoginOrId(credentials.name);

                    credentials.name = thisAccount._id;
                    if (thisAccount.password != credentials.pass) {
                        throw new AuthError('Wrong password');
                    }

                    if (req.params.id == thisAccount.login) {
                        req.params.id = thisAccount._id;
                    }

                    if (req.params.id == thisAccount._id) {
                        return next();
                    }
                    ...
                    const targetAccount = await this.findAccountByLoginOrId(req.params.id);
                    req.params.id = targetAccount._id;
                    const allowedAccess = targetAccount.access;
    ...
    private logHalfSuccessfulResponse(
        req: express.Request,
        eventTypesBefore: any[],
    ...
    private async findAccountByLoginOrId(idOrLogin: string): Promise<any> {
        if (idOrLogin.match(/^\d+$/)) {
            return this.accountsDb.get(idOrLogin);
        } else {
            const docs = await this.accountsDb.query<Account, Account>('web_api_server_v2/by_login', { key: idOrLogin, limit: 2, reduce: false, include_docs: true });
