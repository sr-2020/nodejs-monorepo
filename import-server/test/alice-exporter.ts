...
describe("Model Creation", () => { 
    let ex = new AliceExporter(testCharData01, metadata);
    ...
    it("corporation, salaryLevel, insurance", () => {
        ...
        expect(ex.model.insurance).is.equal(2055);
    ...
    it("Memories", () => {
        ...
        expect(ex.model.memory[0].title).is.equal("Вокруг темно, а я размазываю с...");
        expect(ex.model.memory[0].text).is.equal("Вокруг темно, а я размазываю слёзы по своему лицу. Всё самое лучшее достаётся моему брату, любовь родителей, игрушки, награды. Лучше бы его не было.");

        expect(ex.model.memory[1].title).is.equal("Я вижу застывшую маску умиротв...");
        expect(ex.model.memory[1].text).is.equal("Я вижу застывшую маску умиротворения на лице своего покойного брата. Мне стыдно за свои эгостичные желания. В его смерти нет моей вины, но я продолжаю испытывать жгучее ощущение стыда.");        
    });
