...
let narco = require('./catalogs-data/narco.json');
...
async function saveDocs() {
    if (effects && effects.effects) {
        await saveDatabaseObjects("effects", effects.effects)
            .catch((err) => winston.error(`Problem in import: %s`, inspect(err)));
    }
    ...
    if (narco && narco.pills) {
        await saveDatabaseObjects('pills', narco.pills)
            .catch((err) => winston.error(`Problem in import: %s`, inspect(err)));
    }
